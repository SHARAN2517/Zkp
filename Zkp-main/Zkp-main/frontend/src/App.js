...
import { Shield, Cpu, Activity, Lock, Users, AlertTriangle, CheckCircle, XCircle, Eye, EyeOff, Thermometer, Heart, Zap } from "lucide-react";
...
function App() {
  ...
  const getSensorColor = (sensorType, isPrivacySensitive) => {
    if (isPrivacySensitive) return "text-red-600 bg-red-50";
    switch (sensorType) {
      case "temperature": return "text-orange-600 bg-orange-50";
      case "humidity": return "text-blue-600 bg-blue-50";
      default: return "text-gray-600 bg-gray-50";
    }
  };
    ...
    <div className="min-h-screen bg-gradient-to-br from-slate-50 via-blue-50 to-indigo-50">
      ...
      <div className="max-w-7xl mx-auto px-6 py-8">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-6 mb-8">
          ...
          <Card className="bg-white/60 backdrop-blur-sm">
            <CardHeader className="flex flex-row items-center justify-between pb-2">
              <CardTitle className="text-sm font-medium text-slate-600">Sensor Data</CardTitle>
        ...
        <Tabs defaultValue="devices" className="space-y-6">
          <TabsList className="grid w-full grid-cols-5 bg-white/60 backdrop-blur-sm">
            ...
            <TabsTrigger value="sensors">IoT Sensor Data</TabsTrigger>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
              ...
              <Card className="bg-white/70 backdrop-blur-sm">
                <CardHeader>
                  ...
                  <CardDescription>
                    Real-time sensor readings from connected IoT devices
                  </CardDescription>
                </CardHeader>
                <CardContent>
                  <div className="space-y-3 max-h-96 overflow-y-auto">
                    {sensorReadings.slice(0, 20).map((reading) => (
                      <div key={reading.id} className="flex items-center justify-between p-3 border border-slate-200 rounded-lg bg-white/50">
                        <div className="flex items-center space-x-3">
                          <div className={`p-2 rounded-lg ${getSensorColor(reading.sensor_type, reading.is_privacy_sensitive)}`}>
                          ...
                          <div>
                            ...
                            <p className="text-xs text-slate-500">
                              {reading.sensor_type.replace('_', ' ')} • {new Date(reading.timestamp).toLocaleTimeString()}
                            </p>
                          </div>
                        </div>
                        <div className="flex items-center space-x-2">
                ...
                <CardContent>
                  <div className="space-y-4">
                    <div className="flex justify-between items-center">
                      <span className="text-sm text-slate-600">Temperature Sensors</span>
                      <span className="font-semibold text-orange-600">
                        {sensorReadings.filter(r => r.sensor_type === "temperature").length}
                      </span>
                    </div>
                    <div className="flex justify-between items-center">
                      <span className="text-sm text-slate-600">Heart Rate Monitors</span>
                      <span className="font-semibold text-red-600">
                        {sensorReadings.filter(r => r.sensor_type === "heart_rate").length}
                      </span>
                    </div>
                    <div className="flex justify-between items-center">
                      <span className="text-sm text-slate-600">Humidity Sensors</span>
                      <span className="font-semibold text-blue-600">
                        {sensorReadings.filter(r => r.sensor_type === "humidity").length}
                      </span>
                    </div>
                    <div className="border-t pt-4">
                      <div className="flex justify-between items-center">
                        <span className="text-sm text-slate-600">Privacy-Sensitive Data</span>
                        <Badge className="bg-red-100 text-red-800">
                          {sensorReadings.filter(r => r.is_privacy_sensitive).length}
                        </Badge>
                      </div>
                      <Progress 
                        value={
                          sensorReadings.length > 0 
                            ? (sensorReadings.filter(r => r.is_privacy_sensitive).length / sensorReadings.length) * 100
                            : 0
                        } 
                        className="mt-2 h-2"
                      />
